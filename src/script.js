...
window.addEventListener('resize', () =>
{
    ...
    renderer.setPixelRatio(Math.min(sizes.pixelRatio))
})
...
const createFirework = (count, position, size, texture, radius, color) => {
    ...
    const timeMulipliersArray = new Float32Array(count)
    ...
    for(let i = 0; i < count; i++) {
        ...
        timeMulipliersArray[i] = 1 + Math.random()
    }

    const geometry = new THREE.BufferGeometry()
    geometry.setAttribute('position', new THREE.Float32BufferAttribute(positionsArray, 3))
    geometry.setAttribute('aSize', new THREE.Float32BufferAttribute(sizesArray, 1))
    geometry.setAttribute('aTimeMultiplier', new THREE.Float32BufferAttribute(timeMulipliersArray, 1))
...
const createRandomFirework = () => {
    ...
    const size = 0.1 + Math.random() * 1.0
